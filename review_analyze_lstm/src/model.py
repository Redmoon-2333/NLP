...
class ReviewAnalyzeModel(nn.Module):
    ...
    属性:
        ...
        lstm (nn.LSTM): LSTM循环神经网络层
        linear (nn.Linear): 全连接输出层

    处理流程:
        输入词索引序列 -> 词嵌入 -> LSTM编码 -> 提取最后时刻隐藏状态 -> 线性变换 -> 输出logits
    """
    ...
    def __init__(self, vocab_size, padding_index):
        ...
        初始化说明:
            ...
            - LSTM使用batch_first=True，使输入输出格式为(batch, seq, feature)
    ...
    def forward(self, x: torch.Tensor):
        ...
        功能描述:
            将输入的词索引序列通过Embedding、LSTM和Linear层，
            输出每个样本的情感预测logits值。
